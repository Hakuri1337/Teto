...
public class Scaffold extends Module {
    ...
    public static boolean isFaceVisible(BlockPos pos, Direction face) {
        ...
        Vec3 myEye = mc.player.getEyePosition(Timing.partialTick());
        ...
        double maxDistance = Compat.blockReach(mc.player);
    ...
    public static void holdBlock() {
        ...
        if (block != null) {
            Inv.setSelectedSlot(mc.player.getInventory(), block);
        }
    }
    ...
    public void onTick(EventTick event) {
        ...
        for (Direction direction : d6) {
            ...
            if (isFaceVisible(lastSolid, direction)) {
                //算转头
                var tmp = RotationUtils.aimToPoint(mc.player.getEyePosition(Timing.partialTick()), getFaceCenter(lastSolid, lastState.getShape(mc.level, lastSolid).bounds(), direction));
                //转过去
