...
public final class Transformer implements ClassFileTransformer {
    ...
    private static byte[] dispatch(Class<?> captured, byte[] data) {
        ...
        if (captured == GameRenderer.class) return Render3DHook.transform(data);
        ...
        if (captured == ClientPacketListener.class) return PacketSendHook.transform(KickHook.transform(data));
        return null;
    }
