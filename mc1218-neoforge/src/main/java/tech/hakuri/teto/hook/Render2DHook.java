    ...
    static class MV extends MethodVisitor {
        ...
        public void visitInsn(int opcode) {
            ...
            if (opcode == Opcodes.RETURN) {//注意了我们要hook的方法是void的所以指令是RETURN，如果是返回其他类型，请按鼠标中键点一下RETURN
                mv.visitVarInsn(Opcodes.ALOAD, 1);//1参就是本帧的 GuiGraphics（1.20.1 与 1.21.8 都是第一个参数）
            mv.visitMethodInsn(Opcodes.INVOKESTATIC, Type.getInternalName(EventRender2D.class), ReflectBridge.firstMethodName(EventRender2D.class), Type.getMethodDescriptor(Type.VOID_TYPE, Type.getType(Object.class)), false);
            }
